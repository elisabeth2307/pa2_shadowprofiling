...
class plgHikashopshippingUSPS extends hikashopShippingPlugin
{
	...
	function addRate(&$rates, $type, $parcels, &$rate, $currency, $isInternational) {
		...
		if($isInternational) {
			...
			foreach($usps_rate_arr as $k) {
				...
				if(isset($decoding[$id]) && strcmp($type, $decoding[$id]) == 0) {
					...
					$usps_rates['Service'] = html_entity_decode($k['SvcDescription']);
					$usps_rates['Rate'] += $k['Postage'];
				}
			}
		} else {
			...
			foreach($usps_rate_arr as $k => $v) {
				$usps_rates['Service'] = html_entity_decode($v['MailService']);
				$usps_rates['Rate'] += $v['Rate'];
			}
			if(empty($usps_rates)) {
				$responseError = true;
			}
		...
		if($responseError == false && $usps_rates['Rate'] > 0) {
			if(empty($rates[$type])) {
				$info = new stdClass();
